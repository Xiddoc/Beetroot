...
### Bug fixes
- **`beetroot create`/`register`/`up`/`restore` no longer crash with `KeyError: 'frida'` on a Frida-less instance (#272).** The banners guard every `ports.well_known(...)['frida']` access, so a valid `ports:` list that omits the `frida` service (no `frida:` block) prints an "unsupported" note instead of raising.
- **`beetroot up` no longer advertises a Frida endpoint for `binder: vm` instances (#177).** The post-boot banner prints only the ADB endpoint plus a "Frida unsupported on binder: vm" note for the vm backend, matching `status`/`ls`/`frida-addr`; the redroid banner still shows both ADB and Frida.
- **The strict `binder: host` block on a binderless host now surfaces the `binder: vm` escape hatch (#192).** `plan_binder_runtime` builds the remedy once and reuses it for both the `block` and `warn` branches, so a user who pinned `binder: host` on a binderless box is pointed at the locally-working `binder: vm` path instead of only "use a different host".
- **`beetroot ls` shows the instance directory for `binder: vm` rows instead of `-` (#267).** The directory-backed check accepts `VmDeviceBackend`, so the table PATH column and the JSON `path` key are populated for vm instances (they are directory-backed like redroid).
- **`compose.ps_status` no longer crashes on a non-dict JSON line from `docker compose ps` (#277).** A line that parses to a bare list/string/number is skipped instead of raising an uncaught `AttributeError` through `ls`/`status`/`doctor`.
- **`compose` lifecycle errors now include the daemon's stderr (#276).** `up`/`down` and non-follow `logs` capture *stderr only* (stdout still streams to the terminal) and fold a trimmed stderr tail into the `ComposeError` message so a programmatic caller sees *why* it failed, not just the exit code. `docker compose build` and `logs -f` keep inheriting stdio so their verbose output streams live (capturing it would run them silently).
- **The `binder: vm` guest-rootfs fetch is hardened against a decompression bomb and a truncated download (#274).** zstd decompression aborts past a 64 GiB output ceiling and the streaming download rejects a body shorter than its advertised `Content-Length`, mirroring the frida (#228) and snapshot (#265) guards.
- **`beetroot restore` caps zstd decompression output (#267).** A malicious/corrupt `.tar.zst` is aborted past a generous ceiling instead of filling the disk.
- **The prebuilt guest-kernel download streams to disk instead of buffering the whole `bzImage` in RAM (#246), and the kernel *source* tarball is hashed in 64 KiB chunks (#267).** Both hash incrementally, matching the `rootfs_download` streaming idiom for constrained TCG/CI hosts.
- **The default Docker static bundle is now digest-pinned (#262).** `_DEFAULT_DOCKER_BUNDLE_SHA256` carries the verified sha256 of `docker-27.5.1.tgz`, so the local rootfs bake verifies the default bundle before extracting it into the trusted guest (a `DOCKER_URL`/`DOCKER_VERSION` override stays unverified-with-warning).
- **`binder: vm` warm-boot checkpoints fold the resolved accelerator into their identity (#273).** A savevm checkpoint taken under KVM (`-cpu host`) is no longer resumed under TCG (`-cpu max`) or vice-versa — an accel flip invalidates the checkpoint and cold-boots cleanly instead of attempting an incompatible `-loadvm`.
- **A `binder: vm` warm-resume failure preserves its console log before the cold-boot retry (#267).** The warm-failure trace is copied to a `.warm-fail` sidecar so the error message that points the user at the console log isn't truncated by the fallback boot.
- **`AdbDevice.add_module` raises the typed `AdbNotInstalledError` when `adb` is absent (#275)** instead of a raw `FileNotFoundError`, matching every other adb verb.
- **`registry.add_allocating` bounds the port index by the cap at registration time (#267)**, so an over-cap instance fails cleanly instead of registering an index that crashes every later port resolution.
- **`resources.mem_reservation`/`memswap_limit` are validated against `mem` at load time (#267).** An inverted soft-floor / swap-cap is rejected with a clear error instead of failing opaquely at `docker compose up` (the `-1` unlimited sentinel is honored).
- **`beetroot build --vm-kernel --check` no longer aborts the whole preflight on a malformed `IMAGE_SIZE_MB` (#267)** — it is reported as a preflight problem alongside the others.
