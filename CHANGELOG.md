...
- **`compose` lifecycle errors now include the daemon's stderr (#276).** `up`/`down`/`build` (and non-follow `logs`) fold a trimmed stderr tail into the `ComposeError` message so a programmatic caller sees *why* it failed, not just the exit code; `logs -f` still streams live.
