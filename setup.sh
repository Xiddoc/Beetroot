...
rm -rf .tmp-redroid-script
git clone --depth 1 https://github.com/ayasa520/redroid-script.git .tmp-redroid-script
...
uv run --with requests --with tqdm redroid.py -a 14.0.0 -dmnw

# Note: The script creates an image named: 
# redroid/redroid:14.0.0-mindthegapps-ndk-magisk-widevine
cd ..
rm -rf .tmp-redroid-script
