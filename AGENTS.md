...
- **Adding a new Android version.** The supported set is single-sourced at `config._VALID_ANDROID_VERSIONS` (and `config.DEFAULT_ANDROID_VERSION` for the default). To support Android *N*:
  ...
  3. Update the human-readable enumerations ("11, 12, 13, or 14") in the `config.py`/`builder.py` docstrings, `README.md`, and `docs/reference/config.md`. `tests/test_android_version_extensibility.py` greps the source enumerations against the constant and **fails CI on drift**, and parametrizes both tag-derivation functions across *every* supported version — so a stale docstring or a malformed tag is caught at the unit level.
