...
- **The Claude Code on the web sandbox** (where these agent sessions run) is daemonless and binderless: `docker` CLI present but `dockerd` opt-in (so the daemon-gated tests skip), no host `binder` and no `/dev/kvm` (so `beetroot modes` reports redroid `host/auto` and `vm,KVM` as `unsupported`). The `vm,TCG` path is `needs-setup` (install `qemu-system-x86` + `beetroot build --vm-kernel`) and the adb backend is `needs-setup` (install `android-tools-adb`) — both are reachable but heavy; neither boots out of the box. **Verified `beetroot modes` in-sandbox (2026-06):** `redroid (binder: host/auto)` → `unsupported`, `binder: vm, KVM accel` → `unsupported`, `binder: vm, TCG accel` → `needs-setup`, `adb backend` → `needs-setup`. **Expected skip count:** a clean `uv run pytest` in the sandbox reports **16 skipped** — not 6: the 6 docker-daemon tests *plus* the 10 `test_shell_lint.py` cases that self-skip because `shellcheck`/`shfmt` aren't on `PATH` (install both, or replicate their CI gate with `uvx --from shellcheck-py shellcheck -S style -s sh docker/*.sh docker/vm/*.sh`). **Bottom line: in the sandbox you can run and trust the entire pure-Python suite (with those 16 environment-gated tests skipping, not failing); you cannot boot redroid or a VM, nor run the shell linters, without explicit, heavyweight setup.**
