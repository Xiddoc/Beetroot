...
- `uv run beetroot <verb>` — invoke any CLI verb during development. Verbs: `create`, `register`, `adopt`, `apply`, `up`, `down`, `restart`, `destroy`, `forget`, `ls`, `logs`, `shell`, `env`, `status`, `doctor`, `frida`, `module`, `build`, `snapshot`, `restore`. Run `beetroot <verb> --help` for flags. (With a `uv tool install`-based setup, drop the `uv run` prefix entirely.)
