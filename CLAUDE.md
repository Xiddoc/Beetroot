...
- `uv run beetroot <verb>` — invoke any CLI verb during development. Verbs: `create`, `up`, `down`, `restart`, `destroy`, `ls`, `logs`, `shell`, `env`, `frida`, `module`, `apply`, `build`. Run `beetroot <verb> --help` for flags. (With a `uv tool install`-based setup, drop the `uv run` prefix entirely.)
