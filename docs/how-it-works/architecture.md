...
The `--env-file` supplies all the per-instance variables that the template reads via `${VAR}` substitution: `ADB_PORT`, `FRIDA_PORT`, `MEM_LIMIT`, `CPUS`, `SHM_SIZE`, `DISPLAY_WIDTH`, `DISPLAY_HEIGHT`, `DISPLAY_FPS`, `DISPLAY_GPU`, and `INSTANCE_NAME`. The `.env` file is generated by the CLI from `beetroot.yaml` and never hand-edited.
