...
Starter `beetroot.yaml` files (`default.yaml`, `stealth.yaml`, `no-gapps.yaml`) live in the repo's top-level `examples/` directory and are **not** bundled inside the wheel. They are documentation-only — `beetroot create` doesn't load them. See [the examples guide](../guides/examples.md) for how to use them.
