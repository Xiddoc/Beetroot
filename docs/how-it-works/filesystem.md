...
The Android `/data` partition, bind-mounted read-write. Gigabytes of state; pack into a portable `.tar.zst` with [`beetroot snapshot`](../guides/snapshots.md), or use `cp -a` for a quick same-host rollback.
