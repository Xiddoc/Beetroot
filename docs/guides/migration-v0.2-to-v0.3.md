...
The first `beetroot` command you run on v0.3 after upgrading detects the old v1-shaped registry, renames it to `instances.json.bak` next to its original location, and prints a one-line warning telling you to re-register. You can lean on that auto-detection, or just re-register up front:
...
alpha   0    localhost:5555  localhost:27042  exited    /home/you/alpha
bravo   1    localhost:5565  localhost:27052  exited    /home/you/bravo
```

The `PATH` column is new in v0.3 — it shows the absolute path each instance was registered at. If you see an entry whose path is wrong, `beetroot destroy --no-rm` it and re-register from the correct path. (`destroy --no-rm` removes the registry entry without touching the on-disk directory.)
...
The auto-migration renamed your old registry to `instances.json.bak` next to the old location. Re-register each instance with `beetroot register <path>`.
