...
| `--from-source` | flag | With `--vm-kernel`: always build both guest artifacts locally — compile the kernel from source (~7 min) instead of fetching the matching prebuilt `bzImage`, and bake the rootfs locally instead of fetching the matching prebuilt image. |
...
`--vm-kernel` runs a **host-prerequisite preflight first** (the rootfs build stages `busybox`/`socat`/`iptables-legacy` and shells out to `curl`/`tar`/`ldd`/`mke2fs`, then bakes the redroid image via the Docker daemon). Rather than failing one missing tool at a time, the preflight reports **everything** missing in one pass, each with the apt package (or command) that fixes it, and aborts before building. Run `beetroot build --vm-kernel --check` to see the report without starting a build. If the redroid pull hits a Docker Hub rate limit, point `REDROID_TAR` at a `docker save`d image tarball or use a registry mirror (the daemon check is skipped when `REDROID_TAR` is set).
